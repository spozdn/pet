...
def get_shift_agnostic_loss_naive_1d(targets, predictions):
    ...
    for shift in range(0, targets.shape[0] - predictions.shape[0] + 1):
        now = get_mse_loss(targets[shift : shift + predictions.shape[0]], 
                           predictions)
...
def get_shift_agnostic_loss_naive(predictions, targets):
    ...
    for index in range(targets.shape[0]):
        losses.append(get_shift_agnostic_loss_naive_1d(targets[index],
...
def test_shift_agnostic_loss():
    ...
    shift_agnostic_loss_naive = get_shift_agnostic_loss_naive(predictions,
                                                               targets)
