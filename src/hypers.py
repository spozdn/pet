...
def combine_hypers(provided_hypers, default_hypers, check_dublicated):    
    ...
    if (not result['USE_DIRECT_TARGETS']) and (not result['USE_TARGET_GRADS']):
        raise ValueError("At least one of the direct targets and target grads should be used for fitting")
        
    if (not result['USE_DIRECT_TARGETS']) or (not result['USE_TARGET_GRADS']):
        if (result['DIRECT_TARGET_WEIGHT'] is not None):
            warnings.warn("DIRECT_TARGET_WEIGHT was provided, but in the current calculation, it doesn't affect anything since only one target of direct targets and target grads is used")
