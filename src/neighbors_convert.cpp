...
std::vector<c10::optional<at::Tensor>> process_neighbors(at::Tensor i_list, at::Tensor j_list, at::Tensor S_list, at::Tensor D_list, 
    ...
    TORCH_CHECK(i_list.sizes() == species.sizes(), "i_list and species must have the same size");
    ...
    at::Tensor mask = torch::zeros({n_atoms, max_size}, options_bool);  // Tensor to store the mask
    at::Tensor neighbor_species = torch::zeros({n_atoms, max_size}, options_int);
    ...
    for (int64_t k = 0; k < i_list.size(0); ++k) {
        ...
        if (idx < max_size) {
            ...
            neighbor_species_ptr[i * max_size + idx] = species_ptr[k];
    ...
    if (scalar_attributes.has_value()) {
        return {neighbors_index, neighbors_shift, relative_positions, neighbor_scalar_attributes, nums, mask, neighbor_species, neighbors_pos};
    } else {
        return {neighbors_index, neighbors_shift, relative_positions, c10::nullopt, nums, mask, neighbor_species, neighbors_pos};
    }
}
...
std::vector<c10::optional<at::Tensor>> process_dispatch(at::Tensor i_list, at::Tensor j_list, at::Tensor S_list, at::Tensor D_list, 
    ...
    if (i_list.scalar_type() == at::ScalarType::Int && j_list.scalar_type() == at::ScalarType::Int &&
        ...
        return process_neighbors<int32_t, float>(i_list, j_list, S_list, D_list, max_size, n_atoms, species, scalar_attributes);
        ...
        return process_neighbors<int32_t, double>(i_list, j_list, S_list, D_list, max_size, n_atoms, species, scalar_attributes);
        ...
        return process_neighbors<int64_t, float>(i_list, j_list, S_list, D_list, max_size, n_atoms, species, scalar_attributes);
    } else if (i_list.scalar_type() == at::ScalarType::Long && j_list.scalar_type() == at::ScalarType::Long &&
               S_list.scalar_type() == at::ScalarType::Long && D_list.scalar_type() == at::ScalarType::Double) {
        return process_neighbors<int64_t, double>(i_list, j_list, S_list, D_list, max_size, n_atoms, species, scalar_attributes);
    } else {
